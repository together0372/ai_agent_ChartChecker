...
def create_news_workflow(llm: ChatOllama = None) :
    ...
    quiz = QuizGeneratorAgent(llm) # 퀴즈 생성
    ...
    workflow.add_node("distort", distortion.distortion_detector_graph)
    ...
    workflow.add_edge("distort", "quiz")  # 왜곡 탐색 → 퀴즈 생성
    #ex) workflow.add_edge("collect", "summarize")  # 수집 → 요약
    workflow.add_edge("quiz", END)  # 퀴즈 → 종료
