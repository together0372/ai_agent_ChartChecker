...
A chart/graph MUST have ALL of the following structural elements:
1. AXES: at least one axis with tick marks or scale (X-axis, Y-axis, or both)
2. DATA ELEMENTS: bars, lines, pie slices, dots, or other quantitative visual encodings
3. NUMERIC LABELS or SCALE: numbers indicating data values or axis scale

Images that are NOT charts (even if they have white/plain backgrounds):
- Photos of objects (fruit, animals, people, products) — even on white backgrounds
- Natural scene photographs
- Text documents or screenshots
- Illustrations or drawings without quantitative data
- Flowcharts or org charts (no numeric data axes)

Key distinction: An object on a white background is NOT a chart.
A chart requires axes + data encodings + numbers/labels.

Respond ONLY with this JSON:
```json
{
  "is_chart": true,
  "chart_type": "bar|line|pie|scatter|bubble|histogram|area|heatmap|treemap|other",
  "confidence": 0.95,
  "visual_evidence": ["has X and Y axes with tick marks", "vertical bars with numeric labels"],
  "reason": "brief explanation of structural elements found"
}
```
If NOT a chart:
```json
{
  ...
  "confidence": 0.90,
  "visual_evidence": [],
  "reason": "photo of apple on white background — no axes, no data elements"
}
```"""
