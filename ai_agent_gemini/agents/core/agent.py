...
""").strip()
    ...
    async def _node_observer_mandatory_tools(self, state: AgentState) -> AgentState:
        ...
        extract_sys = textwrap.dedent(f"""
Extract all numeric data from this chart. Chart type hint: {norm_type}.
...
IMPORTANT rules:
- For pie/donut charts: put ALL slice percentages (%) into "pie_pcts" AND "data_values".
  Example: slices 82.9%, 17.1% → "pie_pcts":[82.9,17.1], "data_values":[82.9,17.1]
- For pie/donut: put visible slice angles (degrees) into "pie_angles_deg".
  If not directly visible, estimate from visual size.
- For line charts: estimate each data point Y pixel position (top=0) into "visual_y_px".
- Always fill every field you can read — never leave pie_pcts empty for pie/donut charts.
""").strip()
        ...
        def _build_args(tool_name: str) -> dict | None:
            ...
            if tool_name == "tool_check_multi_pie_sum":
                # pie_pcts 우선, 없으면 data_values 로 폴백
                pcts = _get("pie_pcts") or _get("data_values", [])
                pcts = [v for v in (pcts or []) if isinstance(v, (int, float)) and v > 0]
                if not pcts:
                    # 분석 텍스트에서 퍼센트 패턴 추출 시도
                    import re as _re
                    analysis_text = state.get("observer_analysis", "")
                    found = [float(m) for m in _re.findall(r'\b(\d{1,3}(?:\.\d+)?)\s*%', analysis_text)]
                    pcts  = found if found else []
                if not pcts:
                    return None
                # 단일 파이차트이면 chart_count=1 로 명시 (auto-detect 오탐 방지)
                chart_count = 1 if norm_type in ("pie", "donut", "파이차트", "도넛차트") else 0
                return {"all_pcts": pcts, "chart_count": chart_count}
            ...
            if tool_name == "tool_check_pie_angles":
                ...
                if not pcts:
                    return None
                # 각도 없으면 퍼센트로 이론 각도 계산 (도구 내부에서도 하지만 명시적으로)
                if not angles:
                    total = sum(pcts) or 100
                    angles = [round(p / total * 360, 1) for p in pcts]
                return {"pie_pcts": pcts, "pie_angles_deg": angles}

            if tool_name == "tool_check_selective_annotation":
                idx  = _get("annotated_indices", []) or []
...
""").strip()
        ...
        # Gemini 3.5: temperature deprecated → thinking_level 사용
        llm = ChatGoogleGenerativeAI(
            model=LLM_NAME,
            google_api_key=os.environ.get("GOOGLE_API_KEY") or os.environ.get("GEMINI_API_KEY"),
            max_output_tokens=4000,
            model_kwargs={"generation_config": {"thinking_config": {"thinking_level": THINKING_LEVEL}}},
        )
        llm_with_tools = llm.bind_tools(available)

        try:
            resp = await asyncio.to_thread(
                lambda: _call_with_backoff(
                    lambda: llm_with_tools.invoke(msgs),
                    label="Observer LLM"
                )
            )
        except Exception as e:
        ...
        if getattr(resp, "tool_calls", None):
            ...
            preview = _extract_text(resp.content or "")[:120].replace("\n", " ")
    ...
    async def _node_observer_finalize(self, state: AgentState) -> AgentState:
        ...
        for m in reversed(msgs):
            if isinstance(m, AIMessage) and m.content:
                # Gemini thinking 응답은 content가 list일 수 있음 → 문자열로 변환
                final_content = _extract_text(m.content)
                break
...
""").strip()
        ...
        # bind_tools 대신 JSON 출력만 요청 (math 노드 — 빠른 응답 우선)
        llm = ChatGoogleGenerativeAI(
            model=LLM_NAME,
            google_api_key=os.environ.get("GOOGLE_API_KEY") or os.environ.get("GEMINI_API_KEY"),
        ...
        try:
            resp = await asyncio.to_thread(
                lambda: _call_with_backoff(
                    lambda: llm.invoke(math_msgs),
                    label="Math LLM"
                )
            )
            # Gemini thinking 응답은 content가 list일 수 있음 → 문자열로 변환
            resp_text = _extract_text(resp.content) if resp.content else ""
            math_confirmed = [
                m for m in parse_json(resp_text).get("confirmed_misleaders", [])
